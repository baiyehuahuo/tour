...
func (m *DBModel) GetColumns(dbName, tableName string) ([]*TableColumn, error) {
	rows, err := m.DBEngine.Query("SELECT COLUMN_NAME, DATA_TYPE, COLUMN_KEY, IS_NULLABLE, COLUMN_TYPE, COLUMN_COMMENT FROM COLUMNS WHERE TABLE_SCHEMA = ? AND TABLE_NAME = ?", dbName, tableName)
	if err != nil {
		return nil, err
	}
	if rows == nil {
